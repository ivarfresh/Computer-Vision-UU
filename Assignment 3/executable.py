...
from assignment import set_voxel_positions, generate_grid, get_cam_positions, get_cam_rotation_matrices, draw_trace
...
################################################################################
# prev_centers = []
# prev_colors = [[255, 0, 0],[0, 255, 0],[0, 0, 255],[255, 0, 255]]
# prev_path = []
# path_colors = []
################################################################################
...
def main():
    global color_model, hdrbuffer, blurbuffer, cube, window_width, window_height
    ...
    grid_positions, grid_colors = generate_grid(60,70)
    ...
    while not glfw.window_should_close(window):
        ...
        ######################################################################
        # run constantly and parse prev colors and centers
        positions, colors, next_path, next_path_colors = set_voxel_positions(vc, prev_path, path_colors)
        prev_path = next_path
        path_colors = next_path_colors
        cube.set_multiple_positions(positions, colors)
        ######################################################################

    # Draw a trace of the final voxel path
    draw_trace(prev_path, path_colors)
...
def key_callback(window, key, scancode, action, mods):
    ...
    if key == glfw.KEY_G and action == glfw.PRESS:
        ...
        #########################################################################
        # Process next frame after 'G' is pressed
        # global prev_centers
        # global prev_colors
        # global prev_path
        # global path_colors
